...
class GmediaCore {
	...
	function __construct() {
		...
		add_action( 'init', [ &$this, 'user_capabilities' ], 8 );
		add_action( 'init', [ &$this, 'init_actions' ], 20 );

		add_action( 'clean_gmedia_cache', [ &$this, 'clear_cache' ] );
...
//        add_action( 'deleted_gmedia_term_meta', array( &$this, 'clear_cache' ) );
		add_action( 'created_gmedia_term', [ &$this, 'clear_cache' ] );
		add_action( 'edited_gmedia_term', [ &$this, 'clear_cache' ] );
		add_action( 'deleted_gmedia_term', [ &$this, 'clear_cache' ] );
//        add_action( 'gmedia_clean_object_term_cache', array( &$this, 'clear_cache' ) );

		add_filter( 'get_the_gmedia_terms', [ &$this, 'get_the_gmedia_terms' ], 10, 3 );
	}
	...
	function gm_upload_dir( $create = true ) {
		...
		$uploads = apply_filters( 'gm_upload_dir', [ 'path' => $dir, 'url' => $url, 'error' => false ] );
	...
	function init_actions() {
		...
		if ( ! empty( $gmGallery->options['license_key'] ) && empty( $gmGallery->options['disable_logs'] ) ) {
			add_action( 'gmedia_view', [ &$this, 'log_views_handler' ] );
			add_action( 'gmedia_like', [ &$this, 'log_likes_handler' ] );
			add_action( 'gmedia_rate', [ &$this, 'log_rates_handler' ], 10, 2 );
		}
	}
	 ...
	 * @return string
	 */
	function get_admin_url( $add_args = [], $remove_args = [], $uri = false, $preserve_args = [] ) {
		if ( true === $uri ) {
			$uri = admin_url( 'admin.php' );
		}
		$remove_args = empty( $remove_args ) ? [] : (array) $remove_args;
		$_wpnonce    = [];
		...
		$remove_args = array_unique( array_merge( [ 'doing_wp_cron', '_wpnonce', 'do_gmedia', 'did_gmedia', 'do_gmedia_terms', 'did_gmedia_terms', 'ids' ], $_wpnonce, $remove_args, array_keys( $add_args ) ) );
	...
	function delete_folder( $path ) {
		...
		if ( is_file( $path ) ) {
			...
			if ( ! empty( $files ) && is_array( $files ) ) {
				array_map( [ $this, 'delete_folder' ], $files );
			}
	...
	function gm_get_media_image( $item, $size = '', $cover = true, $default = false ) {
		...
		if ( empty( $item ) ) {
			...
			$images = apply_filters( 'gm_get_media_image', [
				'thumb'    => $image,
				'web'      => $image,
				'original' => $image,
			] );
		...
		if ( 'image' !== $type[0] || $img_cover ) {
			...
			if ( $cover ) {
				...
				if ( ! empty( $cover ) ) {
					...
					$alb = $gmDB->get_gmedia_terms( [ $item->ID ], [ 'gmedia_album' ], [ 'fields' => 'ids' ] );
					if ( ! empty( $alb ) ) {
	...
	function getModulePreset( $module = '', $set_module_callback = '' ) {
		...
		if ( $this->is_digit( $module ) ) {
			...
			if ( $preset && ! is_wp_error( $preset ) ) {
				...
				$module_settings = [ $module => (array) maybe_unserialize( $preset->description ) ];
				$name            = trim( str_replace( '[' . $module . ']', '', $preset->name ) );
			} else {
				return $this->getModulePreset( $set_module_callback );
			}
		} else {
			$preset = $gmDB->get_term( '[' . $module . ']', [ 'taxonomy' => 'gmedia_module', 'global' => '0' ] );
			if ( $preset && ! is_wp_error( $preset ) ) {
				$module          = $preset->status;
				$module_settings = [ $module => (array) maybe_unserialize( $preset->description ) ];
		...
		return [ 'module' => $module, 'settings' => $module_settings, 'name' => $name ];
	}
	...
	function gmcloudlink( $id, $type, $default = false ) {
		...
		if ( ! $default && get_option( 'permalink_structure' ) ) {
			...
			$cloud_link = add_query_arg( [ "$endpoint" => $hashid, 't' => $t[ $type ] ], home_url( 'index.php' ) );
		}
	...
	function gmedia_upload_handler( $file_tmp, $fileinfo, $content_type, $post_data ) {
		...
		if ( $out ) {
			...
			if ( $in ) {
				...
				$return = [ "error" => [ "code" => 101, "message" => __( "Failed to open input stream.", 'grand-media' ) ], "id" => $fileinfo['basename'] ];
			...
			if ( ! $chunks || $chunk === ( $chunks - 1 ) ) {
				...
				if ( $size ) {
					...
					if ( ! empty( $file_meta ) ) {
						...
						if ( ! empty( $file_meta['album'] ) && ( ! isset( $post_data['terms']['gmedia_album'] ) || empty( $post_data['terms']['gmedia_album'] ) ) ) {
							$post_data['terms']['gmedia_album'] = [ $file_meta['album'] ];
						}
					}
				}
				...
				if ( isset( $post_data['terms']['gmedia_category'] ) && ! empty( $post_data['terms']['gmedia_category'] ) ) {
					...
					$post_data['terms']['gmedia_category'] = [];
				}
				...
				$return = [
					"success" => [ "code" => 200, "message" => sprintf( __( 'File uploaded successful. Assigned ID: %s', 'grand-media' ), $id ) ],
					"id"      => $fileinfo['basename'],
				];
				...
				$return = [ "success" => [ "code" => 199, "message" => $chunk . '/' . $chunks ], "id" => $fileinfo['basename'] ];

				return $return;
			}
		} else {
			$return = [ "error" => [ "code" => 102, "message" => __( "Failed to open output stream.", 'grand-media' ) ], "id" => $fileinfo['basename'] ];
	 ...
	 * @return mixed|void
	 */
	function get_file_metadata( $file, $fileinfo = [] ) {
		...
		if ( preg_match( '!^image/!', $fileinfo['mime_type'] ) && file_is_displayable_image( $fileinfo['filepath'] ) ) {
			...
			$metadata['thumb'] = [ 'width' => $imagesize[0], 'height' => $imagesize[1] ];
			$imagesize         = getimagesize( $fileinfo['filepath'] );
			$metadata['web']   = [ 'width' => $imagesize[0], 'height' => $imagesize[1] ];
			if ( $is_file_original ) {
				$imagesize            = getimagesize( $filepath_original );
				$metadata['original'] = [ 'width' => $imagesize[0], 'height' => $imagesize[1] ];
			} else {
	...
	function gmedia_import_files( $files, $_terms, $move, $exists = 0 ) {
		...
		foreach ( $files as $file ) {
			...
			if ( ! is_file( $file ) ) {
				echo $prefix_ko . sprintf( __( 'File not exists: %s', 'grand-media' ), esc_attr( $file ) ) . $eol;
				continue;
			}
			...
			if ( $size ) {
				...
				if ( ! empty( $file_meta ) ) {
					...
					if ( empty( $terms['gmedia_album'] ) && ! empty( $file_meta['album'] ) ) {
						$terms['gmedia_album'] = [ $file_meta['album'] ];
					}
				}
			}
			...
			echo $prefix . $fileinfo['basename'] . ': <span class="ok">' . sprintf( __( 'success (ID #%d)', 'grand-media' ), $id ) . '</span>' . $eol;
	...
	function duplicate_gmedia( $gmid ) {
		...
		$media_data = [
			...
			'terms'       => [
				'gmedia_album'    => $gmDB->get_gmedia_terms( $gmedia->ID, [ 'gmedia_album' ], [ 'fields' => 'ids' ] ),
				'gmedia_category' => $gmDB->get_gmedia_terms( $gmedia->ID, [ 'gmedia_category' ], [ 'fields' => 'ids' ] ),
				'gmedia_tag'      => $gmDB->get_gmedia_terms( $gmedia->ID, [ 'gmedia_tag' ], [ 'fields' => 'ids', 'order' => 'term_order' ] ),
			],
		];
	...
	function recreate_images_from_original( $gmid ) {
		...
		if ( ! empty( $item ) ) {
			...
			$type = $type[0];
			if ( 'image' !== $type ) {
				$out = [ 'error' => $this->alert( 'danger', sprintf( __( '#%d: Not image type', 'grand-media' ), $item->ID ) ) ];
			...
			if ( (int) $item->author !== get_current_user_id() ) {
				if ( ! current_user_can( 'gmedia_edit_others_media' ) ) {
					$out = [ 'error' => $this->alert( 'danger', __( 'You are not allowed to edit others media', 'grand-media' ) ) ];
			...
			if ( ! $filepath_original ) {
				$out = [ 'error' => $this->alert( 'danger', __( 'Original file does not exists', 'grand-media' ) ) ];
			...
			if ( empty( $fail ) ) {
				$out = [ 'msg' => $this->alert( 'info', $success ), 'modified' => $gmedia['modified'] ];
			} else {
				$out = [ 'error' => $this->alert( 'danger', $fail ) ];
			}

		} else {
			$out = [ 'error' => $this->alert( 'danger', sprintf( __( '#%d: No image in database', 'grand-media' ), $gmid ) ) ];
		}
	...
	function app_service( $service ) {
		...
		if ( in_array( $service, [ 'app_updateinfo' ], true ) && ! is_email( $data['site_email'] ) ) {
			...
			if ( in_array( $service, [ 'app_deactivateplugin', 'app_uninstallplugin' ], true ) ) {
				if ( ! empty( $options['site_ID'] ) ) {
					$post_data['site_id'] = $options['site_ID'];
					wp_remote_post( 'https://gmediaservice.codeasily.com/?gmService=' . $service, [
						'method'    => 'POST',
						'timeout'   => 5,
						'blocking'  => false,
						'sslverify' => false,
			...
			if ( in_array( $service, [ 'app_activate', 'app_updateinfo' ], true ) ) {
				$status = 1;
			} else {
			...
			$tagslist     = $gmDB->get_terms( 'gmedia_tag', [
				'hide_empty'    => true,
				'fields'        => 'names_count',
				'orderby'       => 'count',
				'order'         => 'DESC',
			...
			$post_data['hash'] = $hash;
			$gms_post          = wp_remote_post( 'https://gmediaservice.codeasily.com/?gmService=' . $service, [
				'method'    => 'POST',
				'timeout'   => 45,
				//'blocking'  => false,
				'sslverify' => false,
...
//                   $result['gms_post_body'] = $gms_post_body;
	...
	function gmedia_custom_meta_box( $gmedia_id, $meta_type = 'gmedia' ) {
		...
		?>
		<fieldset id="gmediacustomstuff_<?php echo $gmedia_id; ?>" class="gmediacustomstuff" data-metatype="<?php echo esc_attr( $meta_type ); ?>">
			...
			<a href="#newCustomFieldModal" data-gmid="<?php echo absint( $gmedia_id ); ?>" class="newcustomfield-modal label label-primary"><?php _e( 'Add New Custom Field', 'grand-media' ) ?></a>
		</fieldset>
		...
		if ( $meta ) {
			...
			if ( ! empty( $metainfo ) ) {
				foreach ( $metainfo as $key => $value ) {
					...
					$metadata[ $key ] = [ 'name' => $key_name, 'value' => $value ];
				}
			}
		}
	...
	function sanitize_meta_value( $value ) {
		if ( is_array( $value ) && (bool) count( array_filter( array_keys( $value ), 'is_string' ) ) ) {
			...
			foreach ( $value as $key => $val ) {
				...
				$value_return[ $key ] = [ 'name' => $key_name, 'value' => $val ];
			}
		} else {
			$value_return = $value;
		}
		...
		do {
			...
			foreach ( $keymap as $new_key => $original_key ) {
				if ( isset( $array[ $original_key ] ) ) {
					...
					} elseif ( isset( $array[ $new_key ] ) && array_search( $new_key, $keymap, true ) === false ) {
